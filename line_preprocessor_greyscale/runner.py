"""CLI runner for greyscale line preprocessing."""
...
from .processing import initial_line_extraction, process_line_image_greyscale

# Import shared utilities from the original module
import sys as _sys
import os as _os
_sys.path.insert(0, _os.path.dirname(_os.path.dirname(_os.path.abspath(__file__))))
from line_preprocessor.geometry import expand_polygon
...
def main(image_path: str, json_path: str, output_dir: str, pylaia_list_path: str) -> None:
    """
    Main entry point for greyscale line preprocessing pipeline.

    Processes a page image by extracting individual text lines based on Kraken
    segmentation data, normalizing each line to greyscale, and saving them for 
    HTR processing. Creates a list file that can be used for batch processing.
    ...
    4. For each line:
       ...
       - Normalizes (rotates, rectifies, deslants, scales)
       - Applies greyscale normalization (contrast, denoising)
       - Saves the processed line image
       - Adds the path to the output list file
    ...
    Args:
        ...
        pylaia_list_path: Path where the output list file will be written.
            This file contains one absolute path per line, one per processed line.
    ...
    print("Processing and saving greyscale line images...")
    os.makedirs(output_dir, exist_ok=True)
    with open(pylaia_list_path, "w", encoding="utf-8") as output_list_file:
